...
Build a docs-driven-dev release artifact:
...
init = (root / "src" / "docs_driven_dev" / "__init__.py").read_text(encoding="utf-8")
...
ARTIFACT="docdev-$VERSION.tar.gz"
...
TMP_PARENT=$(mktemp -d "${TMPDIR:-/tmp}/docdev-release.XXXXXX")
cleanup() {
  rm -rf "$TMP_PARENT"
}
trap cleanup EXIT INT TERM

RELEASE_ROOT="$TMP_PARENT/docdev-$VERSION"
...
tar -czf "$ARTIFACT_PATH" -C "$TMP_PARENT" "docdev-$VERSION"
...
printf '[docdev package] artifact: %s\n' "$ARTIFACT_PATH"
printf '[docdev package] checksum: %s\n' "$CHECKSUM_PATH"
printf '[docdev package] manifest: %s\n' "$MANIFEST_PATH"
printf '[docdev package] installer: %s\n' "$INSTALLER_SH"
printf '[docdev package] installer: %s\n' "$INSTALLER_PS1"
