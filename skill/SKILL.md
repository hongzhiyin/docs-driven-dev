...
CLI resolution 只使用上面列出的跨机器入口：`docdev` on PATH、Unix-like
`~/.local/bin/docdev`、Windows 新终端里的 `docdev` 或临时
`$HOME\.local\bin\docdev.ps1`。`sync-skill` 同步的是 skill 内容；agent 需要执行 CLI
时直接使用这些 native/PATH 入口。安装不可用的诊断条件是上述入口全部不可用。
