...
它会安装源码 wrapper、运行验证、同步 skill target，并生成兼容 wrapper。默认安装会
force sync。对带 `.docdev-skill-source` marker 的目标，sync 会替换整个 skill 目录，
避免旧文件残留；如果以前用过另一个目标路径，那个旧路径不会被自动清理。
